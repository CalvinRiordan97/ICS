...
public class laneController {
	...
	private HashMap<String,Ticket> sortByPriority(HashMap<String, Ticket> arr){
		//bubble sort
		Ticket t1,t2;
		int p1,p2;
		String str1,str2;
		...
		do{
			...
			for(int i=0; i<lane.getLaneArr().size()-1; i++){
				t1 = arr.get(i);
				p1 = t1.getPriority();
				str1 = t1.getTicketName();
				
				t2 = arr.get(i+1);
				p2 = t2.getPriority();
				str2 = t2.getTicketName();
				if(p1<p2){
					arr.remove(i);
					arr.put(str2, t2);
					arr.remove(i+1);
	...
	public boolean addATicket(Ticket ticket){
		...
		if (booleanArr[2] == true){
			...
			HashMap<String, Ticket> arr  = lane.getLaneArr();
			boolean found = false;
			...
			if(found == false){
				//Adding the ticket 
				arr.put(ticket.getTicketName(),ticket);
	...
	public boolean addATicketToSub(Ticket t){
		...
		if (booleanArr[1] == true && booleanArr[3]== true){
			...
			HashMap<String, Ticket> mainArray = lane.getLaneArr();
	...
	public boolean moveATicketFromSubToMain(Ticket t){
		...
		if (booleanArr[1] == true && booleanArr[3]== true){
			...
			if (subArray.contains(t)){
				...
				mainArray.put(t.getTicketName(),t);
